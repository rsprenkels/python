from aoc import Machine, Haltcode
...
m = Machine(list(map(int, program.split(','))))

# total = 0
#
# for y in range(50):
#     for x in range(50):
#         m.reset_run(list((x,y)))
#         result = m.output.pop()
#         if result == 1:
#             total += 1
#         print(f"{result}", end='')
...
def beam_minmax(y_top):
    x = -1
    found = False
    while not found:
        x += 1
        m.reset_run(list((x, y_top)))
        found = m.output.pop() == 1
    x_min = x
    found = False
    while not found:
        x += 1
        m.reset_run(list((x, y_top)))
        found = m.output.pop() == 0
    x_max = x - 1
    return x_min, x_max

def square_fits(y):
    x1, x2 = beam_minmax(y)
    x3, x4 = beam_minmax(y + 99)
    return x2 - x1 >= 99 and x4 - x3 >= 99 and x2 - x3 >= 99

tval = 1049
print(square_fits(tval))
print(beam_minmax(tval))
